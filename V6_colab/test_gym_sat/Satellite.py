...
class Satellite(InOrbit):
    ...
    def endOfLap(self):
        ...
        if(distance <self.step*5  and self.updated>6):
    ...
    def possibleAction(self,stations):
        ended=  self.endOfLap()
        return  ( self.getAvailableStation(stations) and not(self.isconnecting or self.disconnecting or self.intransmission) and len(self.possibleUnloading)>0 ) or  ended  , ended # if we have possible action or have ended
    ...
    def __str__(self):
        return "Satellite  id : {} | storage capacity : {} | current filling : {} | estimated next filling : {}  | number of tasks : {} | number of stored images : {} | copleted : {} | cumulate priority : {} | visibylity : {} |  coneexions : {}".format(
            self.id,self.storage_capacity,self.fillingrate(),self.estimatedfillingrate(),len(self.tasks), len(self.tokenImages),self.completed,self.totalcompletedPriority,self.allvisibylitys,self.allconnexions)
