...
class Satellite(InOrbit):
    ...
    def __init__(self,width,initial_position,direction,speed,storage_capacity):
        ...
        #tasks parameters
        self.tasks=set(Task.getRadomTasks(250,self))
        self.job=None
    ...
    def updateTasks(self,map=None):
        ...
        for task in self.tasks:
            ...
            if task.expiration <= 0:
                ...
                #we save expired images to be removed at the last
                expired.add(task)
            else :
    ...
    def tryBeginTask(self,map):
        ...
        # if it's possible to do this task we wil make it as current job
            if self.can_do(task):
                ...
                self.job.begin()
                #print("begining task",self.job, self)
                return 
    ...
    def getAvailableStation(self,stations):
        ...
        for station in stations:
            if (self.isAtteignable(station)):
                ...
                self.allvisibylitys += 1 and (not self.isConnected)
    ...
    def getstate(self):
        ...
        state=[np.array([int(self.isConnected),self.availableStation.possibleconnections, self.storage_capacity,self.fillingrate(),len(self.tokenImages)]).reshape(-1),
               tokenimagesstate.mean(0).reshape(-1),
               avilableiamgesstate.mean(0).reshape(-1),
               np.array([len(self.possibleUnloading)]).reshape(-1),
